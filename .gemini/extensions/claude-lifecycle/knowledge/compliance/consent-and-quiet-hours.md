...
That makes quiet-hours enforcement an **import-time responsibility, not a journey-authoring-time one**: it can only be applied where the real trigger timestamp and the user's timezone are both known, which is inside whatever tool runs the journey. Most send platforms have a send-time-window feature for exactly this. The engine's obligation is therefore to hand the requirement over explicitly rather than assume it: every exported journey carrying a push, SMS or WhatsApp step states that its quiet-hours window is unenforced until configured at the destination, so a gap is something the operator declined to close, never something the deliverable quietly omitted.
