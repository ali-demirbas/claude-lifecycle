...
# Parseability comes first, and for every front-matter-bearing file, not just
# skills. Every other check in this script reads front matter with grep, which
# finds `name:` just as happily in a block no YAML parser can load — while the
# things that actually consume it (Claude Code, `npx skills add`, any catalog
# that lists the repo) do parse it, and drop what they cannot read. This check
# found ten lexicon files and a channel template whose front matter had never
# parsed, all of which passed the key checks below.
python3 scripts/check_frontmatter.py \
  skills/*/SKILL.md agents/*.md \
  knowledge/journey-patterns/*.md knowledge/industries/*.md \
  knowledge/lexicons/*.md knowledge/lexicons/locales/*.md \
  knowledge/channels/*.md knowledge/brands/*.md || FAIL=1
...
echo "== 15. Gemini CLI extension matches its sources =="
# .gemini/extensions/ is generated from CLAUDE.md, skills/ and agents/, not
# hand-maintained — same reasoning as the llms-full.txt check below: a source
# file edited without regenerating ships a stale second copy of the plugin
...
echo "== 16. Published Markdown bundle matches its sources =="
